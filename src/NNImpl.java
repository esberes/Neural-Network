...
public class NNImpl {
    ...
    public int predict(Instance instance) {
        ...
        int index = 0;
        double highestOutput = outputNodes.get(0).getOutput();
        for (int i = 1; i < outputNodes.size(); i++)
            if (highestOutput < outputNodes.get(i).getOutput()) {
                index = i;
                highestOutput = outputNodes.get(i).getOutput();
            }
        return index;
    }
    ...
    public void train() {
        for (int b = 0; b < maxEpoch; b++) {
            System.out.print("Epoch: " + b);
    ...
    private double loss(Instance instance) {
        ...
        for (int i = 0; i < outputNodes.size(); i++) {
            crossEntropyLoss += -instance.classValues.get(i) * Math.log(outputNodes.get(i).getOutput());
        }
